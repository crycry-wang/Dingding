...
// 收藏店家
router.post('/likestore', async (req, res, next) => {
  // console.log(req.body)
  db.query('select * from `likestore` where storeID=? && memberID=?', [req.body.storeID, memberId], function (err, results) {
    if (err) console.log("ERR!!");
    if (results != 0) {
      db.query('delete from `likestore` where storeID=? && memberID=?', [req.body.storeID, memberId], function () {
        console.log('取消收藏');
        updateLikestore();

      })
    } else {
      db.query('insert into `likestore`(`memberID`, `storeID`) VALUES (?,?)', [memberId, req.body.storeID], function (err, results) {
        console.log('加入收藏');
        updateLikestore();
      })
  ...
  async function updateLikestore() {
    likeStoreSql = 'SELECT a.memberID,a.storeID,b.storeName from `likestore` a left join `store` b on a.storeID=b.storeID where a.memberID=' + memberId;
